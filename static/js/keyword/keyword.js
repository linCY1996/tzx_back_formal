...
window.onload = function () {
    ...
    var np = new Vue({
        ...
        methods: {
                ...
                axios.post(host + '/route/v1/api/get/keyReply/list', {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        plist.push({
                            ...
                            Id: list[i].Id,
                            channel_name: list[i].channel_name,
                            Key: list[i].Key,
            ...
            Confirms: function () {
                var that = this
                axios.post(host + '/route/v1/api/keyReply/del', {
                    id: parseInt(np.Id),
                    server: server,
                    token: token
                }).then((res) => {
            ...
            copy_test: function (params) {
                // alert("没有接口")
                axios.post(host + '/route/v1/api/keyReply/copy', {
                    from_server: server,
                    id: parseInt(params),
                    is_keep_on: true,
                    to_server: 'test'
            ...
            chaxun: function () {
                window.location.href = '/keywordchaxun?token=' + token + '&keyword=' + np.keyword
            }
        },
        mounted: function () {
            this.showKF(1, 50)
